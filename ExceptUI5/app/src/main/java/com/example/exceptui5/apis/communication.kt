...
class communication:main_thread_api,wiitness_thread_api {
    ...
    override fun zk_verify(threeCret: Three_Cret, time: String): String {//证明者向中心服务器请求零知验证并返回结果证书1和码色
        ...
        var ca2: CA2 = CA2("","","","","","","","12345678","12345678","12345678","12345678")
            ...
            try {
                ...
                while (true) {
                    ...
                    if (receiveFromServer.hasNext()) {//监听公钥并发送要验证的包
                        ...
                        var verifyReq: CA2package = CA2package("","")
    ...
    override fun wiitness_work(
        ...
        var time1 = ""
        var idOfProver = ""
        var mylocationOfInt = Location_Info_Int(1,2,0)
        lateinit var tmpPackageFromProver:JSONObject
        var locationDataOfProver = ""
        lateinit var locationDataOfPolReq:JSONObject
        lateinit var locationDataOfPolReqOfInt:Location_Info_Int
        var px = 1
        var py = 2
        var pz = 0
        ...
        if (true) {//先解密位置信息
            try {
                ...
                mylocationOfInt = locationChangeToInt(tmpMyLocation)
            }
            catch(e:Exception) {
                errordata = "出错位置1.1：\n" + e.toString()
                ke = true
            ...
            try {
                tmpPackageFromProver = JSONObject(packageFromProver)
                time1 = tmpPackageFromProver.getString("time")
                idOfProver = tmpPackageFromProver.getString("idOfprover")
            }
            catch (e:Exception){
                errordata = "出错位置1.2：\n" + e.toString()
                ke = true
            ...
            locationDataOfProver = RSA.decrypt(location1, this.user_PrivateKey)//解密位置信息,得到JSON
            }
            catch (e:Exception){
                errordata = "出错位置1.3：\n" + e.toString()
                ke = true
            ...
            try{
            locationDataOfPolReq = JSONObject(locationDataOfProver)//恢复位置信息对象
            locationDataOfPolReqOfInt = locationChangeToInt(locationDataOfPolReq)
            }
            catch (e:Exception){
                errordata = "出错位置1.4：\n" + e.toString()
                ke = true
            ...
            try{
            px = Math.abs(mylocationOfInt.longitude - locationDataOfPolReqOfInt.longitude)
            py = Math.abs(mylocationOfInt.latitude - locationDataOfPolReqOfInt.latitude)
            pz = Math.abs(mylocationOfInt.altitude - locationDataOfPolReqOfInt.altitude)
            }
            catch (e:Exception){
                errordata = "出错位置1.5：\n" + e.toString()
                ke = true
            ...
            //下面开始发包
            var threadResult:String = "1111"//存放线程的返回结果
                ...
                try {
                    ...
                    while (true) { // 等待计算服务器传来的zk
                        if (RecieveFromServer.hasNext()) {
                            ...
                            val zk2 = zks[1] + "}"
                            //封装zk2
                            var Pol_Resp2: Pol_Resp2 = Pol_Resp2("","","","","")

                            Pol_Resp2.time = time1
                            Pol_Resp2.idOfWitness = idOfProver
                            Pol_Resp2.zk2 = zk2
                            ...
                            Pol_Resp11.zk1 = zk1
                            Pol_Resp11.SignofZk = RSA.encrypt(zk1, this.user_PrivateKey)
        ...
        return "2222"//这句永远不会执行 只是为了适应返回值要求
    }
}
